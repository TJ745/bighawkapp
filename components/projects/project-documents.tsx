...
import { deleteProjectDocument, uploadProjectDocuments } from "@/actions/projects";
import type { ProjectDocumentRow } from "@/lib/data/projects";

type Props = { projectId: string; documents: ProjectDocumentRow[]; canEdit: boolean };

export function ProjectDocuments({ projectId, documents, canEdit }: Props) {
  const router = useRouter();
  const format = useFormat();
  const [uploadOpen, setUploadOpen] = useState(false);
  const [files, setFiles] = useState<PendingFile[]>([]);
  const [uploading, setUploading] = useState(false);
  const [deleting, setDeleting] = useState<ProjectDocumentRow | undefined>();

  async function upload() {
    setUploading(true);
    try {
      const formData = new FormData();
      for (const item of files) formData.append("documents", item.file);
  ...
  async function confirmDelete() {
    if (!deleting) return;
    const result = await deleteProjectDocument(projectId, deleting.id);
    if (!result.success) {
      toast.error(result.error);
      return;
    }
  ...
  return (
    <div className="space-y-4">
      ...
      {documents.length === 0 ? (
        <EmptyState icon={FileText} title="No documents yet" description="Contracts, scope documents and other project files are kept here." />
      ) : (
      ...
      <Dialog open={uploadOpen} onOpenChange={(o) => !uploading && setUploadOpen(o)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Choose one or more files to attach to this project.</DialogDescription>
          </DialogHeader>
          <FilePicker files={files} onChange={setFiles} disabled={uploading} />
          <DialogFooter>
